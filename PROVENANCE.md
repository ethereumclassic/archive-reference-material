...
Every corpus here is copied and held by this repository because losing its upstream would lose
something Ethereum Classic depends on. Each group below states its own ground.

**Two postures, chosen by what would be lost if the upstream vanished tomorrow:** a full vendor,
the whole repository with its history, or an extraction of named files.
`EXTRACTION-historic-clients.md` and `meowsbits/EXTRACTION.md` record the extractions.

**How the tables are measured.** `contents` counts the entries of the tree at the frozen ref,
gitlinks included, and sums the sizes of its blobs; `commits` counts the history reachable from
that ref. Each describes a ref that cannot change, and each was measured from this repository's
own objects.
...
| branch | `etcversion_filled-etcversion_filled-shanghai_2` |
| ref | `06ec708ea7` |
| upstream date | 2023-08-25 |
| vendored | 2026-09-21 |
| mechanism | `git subtree add`, **full history**, 3,182 commits |
| contents | 9,298 files · 8,728 json · 915 MiB |

**Taken at `etcversion_filled-etcversion_filled-shanghai_2`, not at the default branch.** That
branch carries the ETC history. The default branch, `develop`, sits 15 commits behind it, and
archiving `develop` would silently preserve a different corpus. `06ec708ea7` is addressable here,
reachable from `main`.
...
`git diff <ref>..HEAD -- <path>` compares upstream's root-level paths against the prefixed ones
here and reports every file as added. That command reads like a freeze check and is not one: it
reports the whole corpus changed while nothing has been touched.

Compare the trees directly instead:

```sh
git rev-parse '06ec708ea7^{tree}'
git rev-parse 'HEAD:etclabscore/tests'
```

**Two identical hashes is the whole proof**: the archive is bit-for-bit upstream's tree. Verified
2026-09-21: both are `4f98c5a20296d80639bf64c7897a85d5897f2bc1`.

If they ever differ, something edited the archive. That is a defect to revert, not a change to
review.

Gaps and inherited mistakes are answered outside this repository, never by editing these files.
...
The sharpest defect: `Merge` was renamed onto `ETC_Mystique` at scale, and Merge-labeled blocks
...
Read rule sets from the production client, never from a rendered specification.
...
### Consumed as a submodule from this repository

A client whose submodule points at this repository pins the same commit, `06ec708ea7`, and
resolves it because the commit is reachable from `main` as the second parent of this entry's
merge. **The cost is the layout's, and it is not small.** A clone of this repository fetches every
entry's history, not this corpus alone, and a shallow submodule initialization first fetches the
default branch's tip, which is the whole archive's tree, before it fetches the pinned commit.
Weigh that before pointing a submodule here while the upstream still stands.

---

# The Ethereum Classic client lineage — vendored whole, 2026-09-21

**Ten repositories, whole, with history.** Operator decision: this chain's clients are the record
of its eras. Ethereum has had one client present throughout; Ethereum Classic has had core-dev
churn (Classic Geth, then Parity, then multi-geth, then core-geth), and an extraction cannot show
how a client changed across an era.

**Mechanism:** `git subtree add` without `--squash`, of the upstream commit at the named ref,
confirmed published by the upstream itself. Every one is verified by TREE HASH against that
commit: `git rev-parse '<ref>^{tree}'` equals `git rev-parse 'HEAD:<prefix>'` for all ten. A
diff proves nothing here, because subtree relocates the corpus under a prefix and reports every
file as added.

**Totals:** 58,093 commits, 42,709 files, 1,463 MiB.

**Nothing exceeds GitHub's 100 MiB per-file limit**, checked 2026-09-21 across every object
reachable from `main`, largest 62.0 MiB, with the check calibrated at 1 MiB first so that an
empty result means none. That check is not optional: grafted history is permanent, and one
oversized blob would make this repository unpushable forever.
...
| ref | `v2.7.2-stable` @ `55c90d4016505317034e3e98f699af07f5404b63` |
| upstream date | 2020-02-05 |
| vendored | 2026-09-21 |
| mechanism | `git subtree add`, **full history**, 12,342 commits |
| contents | 1,029 files · 13 MiB |

Parity at its last Ethereum Classic-supporting state. **The repository is archived on GitHub** (2020-11-01).
...
|---|---|
| upstream | `https://github.com/openethereum/parity-ethereum` |
| ref | tag `v1.2.2` @ `2cf4549d0181ad1d60fbd3bbe132b599a14a8965` |
| upstream date | 2016-07-16 |
| vendored | 2026-09-21 |
| mechanism | `git subtree add`, **full history**, 4,749 commits |
| contents | 538 files · 4.3 MiB |

The same repository at the DAO fork, published upstream as tag `v1.2.2`; the `-dao` suffix is
this archive's prefix, not a repository name. **This ref is not an ancestor of the one above**: they share root `f7b618cec` and diverge, so vendoring one does not preserve the other. It is the source of `frontier-dogmatic.json`, which sits beside `frontier.json` differing only by the removed `daoHardfork*` block: the declining chain expressed as a config diff.
...
| ref | `master` @ `38865665e94b14d4e9595478789b5d15f925003b` |
| upstream date | 2021-02-27 |
| vendored | 2026-09-21 |
| mechanism | `git subtree add`, **full history**, 12,704 commits |
| contents | 1,567 files · 33 MiB |

The client between Parity and core-geth. Shares root commit `5db3335dc` with core-geth: **one lineage under two names, never a second oracle.**
...
| contents | 28,279 files · 723 MiB |

The multi-geth-era cross-client suite, described upstream as an unfork of `ethereum/tests`. Dormant since 2019 with no stars. The largest entry in this pass at 723 MiB and 28,279 files.
...
| field | value |
|---|---|
| upstream | `https://github.com/besu-eth/besu` |
| ref | `main` @ `eb4248c997cb79cc88db55ead562081a43721a3b`, an ancestor of its tip |
| upstream date | 2026-02-09 |
| vendored | 2026-09-21 |
| mechanism | `git subtree add`, **full history**, 6,443 commits |
| contents | 5,973 files · 61 MiB |

Besu's Ethereum Classic support. **The upstream is a fork organization, not Hyperledger** (`besu-eth/besu`), and the ETC material appears nowhere in `hyperledger/besu`. The org is active, so this is vendored against fork-org risk rather than dormancy. A third independent lineage: root `7dfc2e408`, sharing no commit with geth or Parity, and a third oracle for the difficulty rules. Its ETC surface is six files; a token search for `ecip1010` finds none of them, because the rule lives in `ClassicDifficultyCalculators`.
...
| contents | 528 files · 4.6 MiB |

The Ethereum Classic fork of the end-to-end test harness. Distinct from `ethereum/hive`, which this archive does not hold.
...
# Ethereum Classic tooling and personal-account work — vendored 2026-09-21
...
Every entry is verified by TREE HASH against its upstream commit, with a control confirming the
...
archived and the bot still runs, so a `pushed_at` check reports it as active, and it has
committed past this ref since: the ref is an ancestor of upstream `master`, not its tip. The
...
# The preceding repository's client — vendored 2026-09-21
...
**The negative result means something only because the positive control resolves.** Checked
2026-09-21 against every tag the upstream publishes: all 47 whose commits are ancestors of the
boundary resolve here, and none of the 7 whose commits are not, `v1.12.21`, `v1.12.22` and
`v1.12.23` among them. Nor does the upstream's current `master`.
...
standard independence check reports a shared root and is right to. Classic Geth,
`ethereumproject/go-ethereum`, carries the same root, as every go-ethereum descendant does.
A second shared root, `68ccbefc9`, is NOT lineage at all: it is the early JavaScript library's
first commit, `init` by `obscuren` on 2014-09-30, whose tree holds three files, `.gitignore`,
`ethereum.js` and `index.html`, merged into go-ethereum's history. A shared root is evidence of a
shared repository ancestor and nothing more; check what the root contains before drawing a
lineage from it.
...
Both read `29bf8ae0e7b759e68f6af107647f6ffcfcb27015` at vendoring, 2026-09-21, checked against a
control (the tree of `v1.12.20`'s commit `c2fb44129`, which is `ed3abaab12`), confirming the
comparison can report a mismatch.
...
# Go modules the production client builds against, vendored 2026-09-21
...
organization the tooling section above records as scheduled for deprecation.
...
**Mechanism:** `git subtree add` without `--squash`, of the upstream's default branch, as for every
entry above. **Every branch and every tag upstream is reachable from that ref.** That was read from
each upstream's own `ls-remote` on 2026-09-21, not from a clone's copy of it, so one ref per
repository carries all of them, and every tagged commit resolves here and is reachable from `main`.
Pull-request refs are not branches and are not held; on 2026-09-21, four pull-request heads on
...
Verified 2026-09-21 with go1.26.6: both pinned versions reproduce both of their hashes, which
equal the checksum database's for the same versions, and both controls change them.

### Depending on this repository instead of an upstream

A client pinning one of these modules can keep building if the upstream disappears, by pointing a
`replace` at this repository at the pinned commit's pseudo-version. The WMI fork, recorded in the
next section, works the same way:

```
replace github.com/etclabscore/go-openrpc-reflect v0.0.37 => github.com/ethereumclassic/archive-reference-material v0.0.0-20220829134241-ba5a99fa846d
replace github.com/etclabscore/go-jsonschema-walk v0.0.6 => github.com/ethereumclassic/archive-reference-material v0.0.0-20200501124244-44dea48ac8a4
replace github.com/yusufpapurcu/wmi v1.2.4 => github.com/ethereumclassic/archive-reference-material v0.0.0-20240128142943-6c94d732ac31
```

It works because each pinned commit's own tree has the module at its root, and its `go.mod`
declares the path it replaces, which the Go toolchain requires of a replacement module. No tag is
needed: the commits are reachable from `main` as parents of the subtree merges. The `go.sum` lines
such a `replace` records:

```
github.com/ethereumclassic/archive-reference-material v0.0.0-20200501124244-44dea48ac8a4 h1:VznaTTy3elSE7gKGv4oRwsbQHgEuRNQG6zV7IVfBSZg=
github.com/ethereumclassic/archive-reference-material v0.0.0-20200501124244-44dea48ac8a4/go.mod h1:VdfDY72AFAiUhy0ZXEaWSpveGjMT5JcDIm903NGqFwQ=
github.com/ethereumclassic/archive-reference-material v0.0.0-20220829134241-ba5a99fa846d h1:6JFCC7f5SjTMgPDfBPKXHOgNZJ6rL/Cl4BgXMw8iDKA=
github.com/ethereumclassic/archive-reference-material v0.0.0-20220829134241-ba5a99fa846d/go.mod h1:0404Ky3igAasAOpyj1eESjstTyneBAIk5PgJFbK4s5E=
github.com/ethereumclassic/archive-reference-material v0.0.0-20240128142943-6c94d732ac31 h1:zclga0HG+cnGkrCjFGc39imRUz73X4UJNBIA/yUAvN8=
github.com/ethereumclassic/archive-reference-material v0.0.0-20240128142943-6c94d732ac31/go.mod h1:SBZ9tNy3G9/m5Oi98Zks0QjeHVDvuK0qfxQmPyzfmi0=
```

Verified 2026-09-21 with go1.26.6, from a copy of this repository holding `main` and no tags: each
pseudo-version resolves to its pinned commit and produces the lines above, and a program importing
what the production client imports builds for Linux and Windows inside that client's module graph
with the three `replace` lines in place. **The cost is the layout's**: resolving a pseudo-version
directly from this repository fetches every entry's history, not the module alone.
...
# The WMI fork the production client compiles into Windows builds, vendored 2026-09-21
...
Verified 2026-09-21 with go1.26.6: `v1.2.4` reproduces both hashes below, which equal the checksum
database's, and moving the tag to `v1.2.3`'s commit changes them.
...
covers every tag. Pull-request refs are not held; on 2026-09-21, four pull-request heads pointed at
